...
class TestHFTPErrors(TestBase):
    ...
    def test_file_not_found_slice(self):
        c= self.new_client()
        c.send('get_slice does_not_exist 3 10')
        status, message = c.read_response_line(TIMEOUT)
        self.assertEqual(status, constants.FILE_NOT_FOUND,
                         "El servidor no contestó 202 ante un archivo inexistente")
        c.close()

    def test_negative_offset_or_size(self):
        f = open(os.path.join(DATADIR, 'bar'), 'w')
        f.write('data')
        f.close()
        c= self.new_client()
        c.send('get_slice bar -3 -10')
        status, message = c.read_response_line(TIMEOUT)
        self.assertEqual(status, constants.INVALID_ARGUMENTS,
                         "El servidor no contestó 201 ante numeros negativos de offset y size")
        c.close()
