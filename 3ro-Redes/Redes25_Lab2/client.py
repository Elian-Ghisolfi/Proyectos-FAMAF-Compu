...
from constants import (
    DEFAULT_ADDR, DEFAULT_PORT, CODE_OK, EOL, FILE_NOT_FOUND
)
...
class Client(object):
    ...
    def close(self) -> None:
        ...
        if self.status != CODE_OK:
            logging.warning(
                "Warning: quit no contesto ok, sino '%s'(%s)'."
                % (message, self.status)
            )
    ...
    def read_line(self, timeout: Optional[float] = None):
        """
    ...
    def read_fragment(self, length: int) -> ByteString:
        """
    ...
    def get_metadata(self, filename: str) -> int:
        """
    ...
    def get_slice(self, filename: str, start: int, length: int) -> None:
        ...
        if self.status == CODE_OK:
            ...
            logging.warning(
                "El servidor indico un error al leer de %s." % filename
            )
    ...
    def retrieve(self, filename: str) -> None:
        ...
        if self.status == CODE_OK:
            ...
            logging.warning(
                "No se pudo obtener el archivo %s (code=%s)."
