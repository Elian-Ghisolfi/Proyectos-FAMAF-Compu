 ...
 * (salida). filename: cadena con el nombre del archivo de la redirección o
 * NULL si no se quiere redirección. El TAD se apropia de la referencia.
 * Requires: self!=NULL
 */
...
/* Transformador de comando simple a argumentos válidos para la ejecución con execvp()
 *  self: comando simple a convertir.
 *  Returns: un arreglo de strings representando cada argumento del comando.
 *    El llamador es dueño del string resultante.
 *  Requires: self != NULL
 *  Ensures: result != NULL
 */
...
/* Devuelve el n-ésimo comando de un pipeline.
 *   self: pipeline a iterar.
 *   n: el índice a usar.
 *   Returns: el n-ésimo comando del pipeline si n es una posición válida en el mismo.
 *    Si no, devuelve NULL
 * Requires: self!=NULL
 * Ensures: command != NULL || (command == NULL && n > pipeline_length(self))
 */
