...
queue queue_enqueue(queue q, queue_elem e) {
    ...
    struct s_node *last_aux = q->last;
    if (q->first==NULL) {
        q->first = new_node;
        q->last = new_node;
        q->size = 0u;
    } else {
        last_aux->next = new_node;
        q->last = new_node;
    }
    q->size ++;
    assert(invrep(q) && !queue_is_empty(q));
...
queue queue_disscard(queue q, unsigned int n){
    assert(invrep(q));
    assert(n <= q->size && q->first != NULL); 

    struct s_node *aux_n = q->first;
    struct s_node *pre_aux = q->first;

    if (n == 0u){
        q->first = pre_aux->next;
        destroy_node(aux_n);  
    }else {
